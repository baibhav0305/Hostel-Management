...
const { Admin, User, Hostel, Student } = require("../models");
...
const registerAdmin = async (req, res) => {
  try {
    ...
    try {
      ...
      let isAdmin = await Admin.findOne({ email: admin.email });
      ...
      res.json({ success, token, admin: isAdmin });
    } catch (error) {
      console.log(error);
...
const createHostel = async (req, res) => {
  try {
    let success = false;
    const { hostelDetails, isSuper } = req.body;
